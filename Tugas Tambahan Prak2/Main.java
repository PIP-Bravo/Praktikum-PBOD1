...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    arispuji.viewTaughtCourses();
    
    // Lanjutan penerapan fungsi set dan get
    jafhan.setAge(21);
    jafhan.setAddress("Jl.Mario Bros");
    System.out.println(jafhan.getStudentAge());
    System.out.println(jafhan.getStudentAddress());
    ...
    beta.setAge(26);
    System.out.println(" ");
    System.out.println(beta.getLectureAge());
    beta.viewTaughtCourses();
		  ...
		  switch (incoming) {
			  case "1":
				  for(int i = 0; i<=(allStudents.size()-1); i+=1) {
						System.out.print(allStudents.get(i).getStudentName()+" ");
				  }
				  System.out.println(" ");
		          System.out.print("Pilih mahasiswa menggunakan indeks (mulai 0) : ");
		          int name = scanner.nextInt();
		          System.out.println(" ");
		          allStudents.get(name).viewEnrolledCourses();
		          break;
			  case "2":
			  ...
			  case "3":
				  for(int i = 0; i<=(allLecturers.size()-1); i+=1) {
						System.out.print(allLecturers.get(i).getLectureName()+" ");
				  }
				  System.out.println(" ");
		          System.out.print("Pilih dosen menggunakan indeks (mulai 0) : ");
		          int name2 = scanner.nextInt();
		          System.out.println(" ");
		          allLecturers.get(name2).viewTaughtCourses();
		          break;
			  case "4":
				  for(int i = 0; i<=(allStudents.size()-1); i+=1) {
						System.out.print(allStudents.get(i).getStudentName()+" ");
